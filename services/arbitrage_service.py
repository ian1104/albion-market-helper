    ...
    def calculate(self, buy_price: float, sell_price: float, quantity: int) -> dict[str, float | int | None]:
        ...
        if not self.cost_model.configured:
            return {
                ...
                "gross_revenue": gross_revenue, "purchase_cost": purchase_cost,
                "gross_profit": gross_profit, "fees": None, "transport_cost": None,
                "estimated_net_profit": None, "profit_per_unit": None,
        ...
        return {
            "quantity": quantity, "buy_price": buy_price, "sell_price": sell_price,
            "gross_revenue": gross_revenue, "purchase_cost": purchase_cost,
            "gross_profit": gross_profit, "fees": fees, "transport_cost": transport,
            "estimated_net_profit": net, "profit_per_unit": net / quantity,
            "gross_roi_percent": gross_profit / purchase_cost * 100.0,
...
class ArbitrageService:
    ...
    def opportunities(self, item_id: str | None = None, quality: int = 1, quantity: int = 1,
                      min_spread_percent: float | None = None, min_roi: float | None = None,
                      min_profit: float | None = None, sort: str = "roi", limit: int = 20,
                      cost_model: CostModel | None = None,
                      freshness_max_age_minutes: float = FRESH_DATA_MAX_AGE_MINUTES,
                      historical_range_start: str | None = None, historical_range_end: str | None = None) -> list[dict[str, Any]]:
        ...
        if sort not in {"profit", "roi", "spread", "stability", "freshness"}:
            raise ValueError("sort must be one of: profit, roi, spread, stability, freshness")
        ...
        for current_item, item_rows in grouped.items():
            ...
            for buy in usable_buy:
                for sell in (r for r in usable_sell if r["city"] != buy["city"]):
                    ...
                    if min_roi is not None and roi_for_filter < min_roi:
                        continue
                    if min_profit is not None and (net_profit is None or net_profit < min_profit):
                        continue
                    ...
                    result.append({
                        "status": status, "server": self.server, "item_id": current_item, "quality": quality,
                        ...
                        "spread": {"absolute": spread, "percent": spread_pct}, "profit": profit,
        ...
        def key(o: dict[str, Any]):
            hist, profit = o["historical"], o["profit"]
    ...
    def calculate(self, buy_price: float, sell_price: float, quantity: int, cost_model: CostModel | None = None) -> dict[str, Any]:
        ...
        return {"buy_price": buy_price, "sell_price": sell_price,
                ...
                "profit": profit, "status": "valid" if cost_model.configured else "cost_model_missing", "server": self.server}
