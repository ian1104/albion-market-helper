...
class CraftingStrategy:
    ...
    def __init__(self, database: Database, recipe_repository: RecipeRepository | None = None, production_rules: ProductionRuleProvider | None = None):
        ...
        if recipe_repository is not None:
            ...
            self.recipes = None
        self.production_rules = production_rules or ProductionRuleProvider(Path(__file__).resolve().parent.parent / "data" / "production_rules.json")
    ...
    def _evaluate_recipe(self, recipe: Recipe, *, server: str, city: str, batch_size: int, capital: float | None,
                         use_focus: bool, station_fee: float | None, selling_fee: float, transaction_tax: float,
                         daily_bonus: float | None) -> BusinessOpportunity | None:
        ...
        return BusinessOpportunity(
            strategy_id="crafting", title=f"Craft {recipe.output_item_id} in {city}", server=server, location=city,
            required_capital=upfront, available_capital=capital, capital_utilization_percent=utilization,
            required_quantity=batch_size, executable_quantity=produced, expected_revenue=revenue, expected_cost=expected_cost,
            expected_profit=profit, roi_percent=roi, profit_per_hour=profit_hour, risk=self.definition.risk_level,
            liquidity="available", confidence=confidence, freshness=self._freshness(rows),
    ...
    def _legacy_evaluate(self, **inputs: Any) -> list[BusinessOpportunity]:
        server = inputs.get("server"); city = inputs.get("city"); output_item_id = inputs.get("output_item_id")
        materials_input = inputs.get("materials"); batch_size = inputs.get("batch_size", 1); return_rate = inputs.get("return_rate")
        crafting_fee = inputs.get("crafting_fee", 0.0); selling_fee = inputs.get("selling_fee", 0.0); transaction_tax = inputs.get("transaction_tax", 0.0)
        time_minutes = inputs.get("time_minutes"); capital = inputs.get("capital")
        ...
        # Legacy caller contract treats crafting_fee as the fee for the evaluated batch.
        upfront = material_cost + crafting_fee
        ...
        cost = effective + crafting_fee + fees
        profit = revenue - cost
        roi = profit / upfront * 100 if upfront > 0 else None
        profit_hour = profit / (time_minutes / 60) if time_minutes else None
        utilization = None if capital is None or capital <= 0 else upfront / capital * 100
        return [BusinessOpportunity(
            strategy_id="crafting", title=f"Craft {output_item_id} in {city}", server=server, location=city,
            required_capital=upfront, available_capital=capital, capital_utilization_percent=utilization,
            required_quantity=batch_size, executable_quantity=batch_size, expected_revenue=revenue, expected_cost=cost,
            expected_profit=profit, roi_percent=roi, profit_per_hour=profit_hour, risk=self.definition.risk_level,
            liquidity="available", confidence="MEDIUM", freshness=self._freshness(rows),
    ...
    def evaluate(self, **inputs: Any) -> list[BusinessOpportunity]:
        ...
        if server not in SUPPORTED_SERVERS or not city or self.recipes is None: return []
        batch_size = int(inputs.get("batch_size", 1)); use_focus = bool(inputs.get("use_focus", False))
        station_fee = inputs.get("station_fee", inputs.get("crafting_fee")); selling_fee = float(inputs.get("selling_fee", 0)); transaction_tax = float(inputs.get("transaction_tax", 0)); daily_bonus = inputs.get("daily_bonus")
